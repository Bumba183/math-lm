...
(function () {
  ...
  function openSettings(initialTab) {
    ...
    function renderPanelTab() {
      ...
      stashHint.innerHTML = '<b>Тип позиции.</b> Соберите список типов со страницы, где они перечислены ' +
        ...
        'каждому приоритет от 1 до 5. Что он значит — решаете вы; на оценку риска он влияет ровно так: ' +
        'середина — 3 и ноль баллов, каждый шаг вверх +5, каждый шаг вниз −5.';
      ...
      function renderStash() {
        ...
        stash.values.forEach((value) => {
          ...
          [['', 'не задан'], ['1', 'приоритет 1'], ['2', 'приоритет 2'], ['3', 'приоритет 3'],
           ['4', 'приоритет 4'], ['5', 'приоритет 5']].forEach((pair) => {
  ...
  function aroundParts(facts) {
    ...
    if (rank) {
      const points = (rank - 3) * 5;                       // приоритет выше середины — плюс к риску
      if (points) {
        out.push({ key: 'Тип клада: ' + type, group: 'around', points: points,
                   note: 'приоритет ' + rank + ' из 5, середина — 3' });
      }
    }
